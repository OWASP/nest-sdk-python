...
class Releases(BaseSDK):
    def list_releases(
        ...
        *,
        ...
        :param organization: Organization that releases belong to (filtered by repository owner)
        :param repository: Repository that releases belong to
        ...
        if utils.match_response(http_res, "200", "application/json"):
            return unmarshal_json_response(models.PagedRelease, http_res)
    ...
    def get_release(
        ...
        *,
        ...
        req = self._build_request(
            method="GET",
            path="/api/v0/releases/{organization_id}/{repository_id}/{release_id}",
            base_url=base_url,
            url_variables=url_variables,
            request=request,
            request_body_required=False,
        ...
        if utils.match_response(http_res, "200", "application/json"):
            return unmarshal_json_response(models.ReleaseDetail, http_res)
        if utils.match_response(http_res, "404", "application/json"):
            response_data = unmarshal_json_response(models.ReleaseErrorData, http_res)
            raise models.ReleaseError(response_data, http_res)
        if utils.match_response(http_res, "4XX", "*"):
            http_res_text = utils.stream_to_text(http_res)
            raise models.NestAPIError("API error occurred", http_res, http_res_text)
        if utils.match_response(http_res, "5XX", "*"):
